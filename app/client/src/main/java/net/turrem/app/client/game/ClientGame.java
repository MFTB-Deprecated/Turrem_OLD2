...
public class ClientGame
{
	...
	public void renderSelect()
	{
		...
		if (side != null)
		{
			...
			int[] inds = ClientGame.vertOffinds[side.ind];
			...
			for (int i = 0; i < 4; i++)
			{
				this.renderSelectVert(posx, posy, posz, ClientGame.vertOffs[inds[i]], i, addx, addy, addz);
			}
			GL11.glEnd();
			this.terrselect.end();
			this.toppin.start();
			GL11.glBegin(GL11.GL_QUADS);
			inds = ClientGame.vertOffinds[EnumDir.YUp.ind];
			posy = this.theWorld.getHeight(posx, posz) - 1;
			for (int i = 0; i < 4; i++)
			{
				this.renderSelectVert(posx, posy, posz, ClientGame.vertOffs[inds[i]], i, 0.0F, off, 0.0F);
			}
			GL11.glEnd();
			this.toppin.end();
		}
