package net.turrem.app.client.asset.obj;
...
public class OBJLoader
{
	...
	private HashMap<OBJVertex, Integer> vertsmap;
	
	private int[][] faces;
	private OBJVertex[] verts;
	...
	public void load()
	{
		...
		for (int[][] faceinds : this.fs)
		{
			...
			{
				int[] inds = faceinds[i];
				OBJVertex vert = this.getVertex(inds);
		...
		this.verts = new OBJVertex[this.vertsmap.size()];
		for (Entry<OBJVertex, Integer> ent : this.vertsmap.entrySet())
		{
			this.verts[ent.getValue()] = ent.getKey();
		}
	}
	
	public OBJVertex[] getVerts()
	{
		return this.verts;
	}
	...
	private OBJVertex getVertex(int[] vert)
	{
		Vector3f v = null;
		Vector2f vt = null;
		Vector3f vn = null;
